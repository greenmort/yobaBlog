...
const renderField = ({ input, label, type, meta: { touched, invalid, error, warning } }) => (
  <div>
    ...
    <div>
      ...
      {touched &&
        ((error && <span className="error">{error}</span>) || (warning && <span className="warning">{warning}</span>))}
    </div>
  </div>
);
...
class PostCreate extends Component {
  onSubmit = props => {
    this.props.createPost(props).then(() => this.props.history.push('/'));
  };
      ...
      <form onSubmit={handleSubmit(this.onSubmit)}>
        ...
        <div className="form-group">
          <label htmlFor="content">
            Content
            <Field name="content" component="textarea" className="post-area form-control" />
          </label>
        </div>
